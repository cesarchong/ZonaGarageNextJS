...
export default function BestSellers() {
  ...
  const [promotionData, setPromotionData] = useState({
    ...
    fecha_fin: new Date(new Date().setMonth(new Date().getMonth() + 1)).toISOString().split("T")[0],
  })
  ...
  const createPromotion = () => {
    ...
    const newPromotion: Promocion = {
      ...
      productos_incluidos: selectedProducts.map(p => p.id),
            ...
            {promotions.length === 0 ? (
              ...
              <div className="space-y-4">
                {promotions.map((promo) => {
                  ...
                  const promoProducts = allProducts.filter((p) => promo.productos_incluidos?.includes(p.id))
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {promo.descuento}% de descuento • {promoProducts.length} productos
                          </p>
                        </div>
                        <span
                          className={`text-xs px-2 py-1 rounded ${promo.activa ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                        >
